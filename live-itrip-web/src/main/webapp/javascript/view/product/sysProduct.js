...
function funCreateHtmlFile(productId) {
    if (confirm("确定要生成Html静态页面吗?")) {
        console.log("create html Product id:" + productId);

        var jsondata = {
            'op': 'product.createHtml',
            'token': parent.token,
            'productId': productId
        };
        ...
        parent.execAjaxData("/system/view/product.action", JSON.stringify(jsondata), true
            , function (response) {
                ...
                // success
                if (response.code == 0) {
                    parent.notifySuccess('操作成功', '');
                } else {
                    parent.notifyDanger('操作失败', response.msg);
                }
            }, function () {
                // complete
            });
